 ...
 *********************************************************************************/  
       ...
       case 0x0F:  // Serial Write to LabVIEW
         ...
         break;                         //Given to bites to be read of arduino READ tool
      ...
      case 0x11: //RF send
         ...
         {
         char msg[13]="";
         for(int i=0;i<command[2];i++)     //String is recieved from LW and sent to RF.
         msg[i]=command[i+3];             //Max length 11 charecters 
         vw_send((uint8_t *)msg, strlen(msg));
         vw_wait_tx(); // Wait until the whole message is gone
         }
         break;
      case 0x12: //Joystick send
         if(command[2]>0)
         {
         char msg[15]="";
         for(int i=0;i<9;i++)     //Joystick info is recieved from LW and sent to RF.
         msg[i]=command[i+2];             //Max length 11 charecters 
         vw_send((uint8_t *)msg, strlen(msg));
         vw_wait_tx(); // Wait until the whole message is gone
         }
         break;
